			   ...
			   struct sk_buff *skb)
{
	...
	if (ct->status & statusbit) {
		struct nf_conntrack_tuple target;
            ...
            //yzg: forexample: original flow: 192.168.1.100:100 -> 202.1.1.200:200
		...
		l4proto = __nf_nat_l4proto_find(target.src.l3num,
						target.dst.protonum);
            //yzg: target contains the new ip and port.					
		if (!l3proto->manip_pkt(skb, 0, l4proto, &target, mtype))  //nf_nat_l3proto_ipv4 registered. call nf_nat_ipv4_manip_pkt
			return NF_DROP;
	}
	return NF_ACCEPT;
}
